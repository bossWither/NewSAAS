const translations = {
  bg: {
    // Navbar
    "nav_features": "Функции",
    "nav_how_it_works": "Как работи",
    "nav_pricing": "Цени",
    "nav_get_demo": "Заяви Демо",

    // Hero
    "hero_badge": "🚀 Бъдещето на управлението на имоти",
    "hero_title_1": "Управлявайте финансите на сградата ",
    "hero_title_gradient": "без усилия",
    "hero_subtitle": "Bigla е най-добрата SaaS платформа за касиери на сгради. Следете такси, управлявайте разходи и дръжте живущите информирани с несравнима прозрачност.",
    "hero_btn_trial": "Заяви Демо",
    "hero_btn_features": "Разгледай Функциите",
    "hero_stat_1_val": "100%",
    "hero_stat_1_lbl": "Прозрачност",
    "hero_stat_2_val": "0",
    "hero_stat_2_lbl": "Екселски таблици",
    "hero_stat_3_val": "24/7",
    "hero_stat_3_lbl": "Облачен достъп",

    // Features
    "feat_title": "Защо да изберете Bigla?",
    "feat_subtitle": "Създадена за касиери и обичана от живущите. Спрете да преследвате плащания и започнете да управлявате по-умно.",
    "feat_1_title": "Проследяване в реално време",
    "feat_1_desc": "Моментални актуализации кой е платил и кой дължи. Синхронизацията в реално време означава, че винаги виждате най-новите данни.",
    "feat_2_title": "Управление на разходите",
    "feat_2_desc": "Лесно записвайте разходи за сградата, ремонти и такси за почистване. Дръжте всички отговорни с ясни финансови баланси.",
    "feat_3_title": "Отчети с един клик",
    "feat_3_desc": "Генерирайте подробни месечни отчети с един клик и ги копирайте директно в WhatsApp или Viber.",

    // How it Works
    "hiw_title": "Как работи?",
    "hiw_subtitle": "Започнете да управлявате сградата си за по-малко от 5 минути.",
    "hiw_step_1_title": "1. Регистрация",
    "hiw_step_1_desc": "Регистрирайте вашата сграда и настройте основните параметри и месечни такси.",
    "hiw_step_2_title": "2. Добавяне на апартаменти",
    "hiw_step_2_desc": "Въведете списъка с апартаменти и индивидуалните такси за всеки от тях.",
    "hiw_step_3_title": "3. Проследяване",
    "hiw_step_3_desc": "Отбелязвайте плащания с един клик и генерирайте прозрачни отчети за живущите.",

    // Calculator
    "calc_title": "Спестете време и усилия",
    "calc_slider_label": "Брой апартаменти: ",
    "calc_time_saved_1": "Спестява ви ~",
    "calc_time_saved_2": " събиране на такси и попълване на таблици всеки месец.",
    "calc_paper_saved_1": "Спестява ",
    "calc_paper_saved_2": " хартиени бележки и квитанции годишно.",
    "calc_price_label": "Месечна такса: ",

    // Pricing
    "pricing_title": "Прости и прозрачни цени",
    "pricing_subtitle": "Изберете план, който отговаря на нуждите на вашата сграда.",
    
    "price_demo_title": "Демо Версия",
    "price_demo_price": "Безплатно",
    "price_demo_feat_1": "Тествайте всички функции",
    "price_demo_feat_2": "До 5 апартамента",
    "price_demo_feat_3": "Ограничени отчети",
    "price_demo_btn": "Заяви Демо",

    "price_pro_title": "Премиум",
    "price_pro_price": "От 5 € / мес.",
    "price_pro_feat_1": "5 € за първите 10 ап., +0.15 € за всеки следващ",
    "price_pro_feat_2": "Пълни отчети за Viber/WhatsApp",
    "price_pro_feat_3": "Приоритетна поддръжка",
    "price_pro_btn": "Вземи Премиум",

    // FAQ
    "faq_title": "Често задавани въпроси",
    "faq_subtitle": "Имате въпроси? Ето най-често срещаните отговори.",
    "faq_q1": "Колко време отнема първоначалната настройка?",
    "faq_a1": "Настройката отнема по-малко от 5 минути. Просто въвеждате броя апартаменти и месечните такси, и сте готови.",
    "faq_q2": "Трябва ли живущите да инсталират приложение?",
    "faq_a2": "Не! Живущите получават ясните си месечни отчети директно във Viber или WhatsApp като текстови съобщения.",
    "faq_q3": "Сигурни ли са данните на сградата?",
    "faq_a3": "Абсолютно. Всички данни се съхраняват сигурно в облака с криптиране, така че само вие имате достъп до тях.",
    
    // Contact / Demo Form
    "contact_title": "Заявете вашето демо",
    "contact_subtitle": "Оставете вашите контакти и ние ще се свържем с вас, за да ви покажем как Bigla може да помогне на вашата сграда.",
    "contact_lbl_name": "Име и Фамилия",
    "contact_lbl_email": "Имейл адрес",
    "contact_lbl_phone": "Телефонен номер",
    "contact_btn_submit": "Изпрати Заявка",
    "contact_success": "Благодарим ви! Ще се свържем с вас скоро.",

    // Testimonials
    "testi_title": "Какво казват касиерите",
    "testi_subtitle": "Вижте как Bigla помага на сгради като вашата.",
    "testi_1_text": "\"Събирането на такси ми отнемаше цял уикенд. Сега го правя за 15 минути чрез Viber отчети!\"",
    "testi_1_author": "Георги Димитров",
    "testi_1_role": "Касиер, 45 апартамента",
    "testi_2_text": "\"Живущите най-после виждат къде отиват парите. Прозрачността е на 100%.\"",
    "testi_2_author": "Мария Иванова",
    "testi_2_role": "Касиер, 20 апартамента",
    "testi_3_text": "\"Забравих за екселските таблици. Всичко е автоматизирано и лесно за използване.\"",
    "testi_3_author": "Петър Стоянов",
    "testi_3_role": "Домоуправител, 32 апартамента",
    
    // Cookie Consent
    "cookie_msg": "Този сайт използва бисквитки, за да ви осигури най-доброто изживяване.",
    "cookie_accept": "Приемам",
    "cookie_decline": "Отказвам",

    // Footer
    "footer_desc": "Умно управление на имоти и такси за модерни общности.",
    "footer_github": "Виж в GitHub",
    "footer_privacy": "Поверителност",
    "footer_terms": "Общи условия",
    "footer_rights": "© 2026 Bigla SaaS. Всички права запазени.",

    // Legal Pages
    "legal_privacy_title": "Политика за поверителност",
    "legal_privacy_last_updated": "Последно обновена: 28 Май 2026",
    "legal_privacy_h2_1": "1. Въведение",
    "legal_privacy_p1": "Ние в Bigla се отнасяме сериозно към вашата поверителност. Тази политика обяснява как събираме, използваме и защитаваме вашата лична информация при използване на нашата платформа.",
    "legal_privacy_h2_2": "2. Събиране на данни",
    "legal_privacy_p2": "Събираме само информация, необходима за предоставяне на услугата: имена, имейли и данни за апартамента. Данните за плащанията се обработват сигурно от трети страни.",
    "legal_privacy_h2_3": "3. Споделяне на информация",
    "legal_privacy_p3": "Никога не продаваме вашите данни на трети страни. Всички данни се съхраняват криптирани на сигурни сървъри в ЕС.",
    "legal_privacy_h2_4": "4. Вашите права (GDPR)",
    "legal_privacy_p4": "Имате право да поискате достъп до вашите данни, тяхното коригиране или изтриване по всяко време. Моля, свържете се с нас за съдействие.",
    "legal_privacy_h2_5": "5. Бисквитки (Cookies)",
    "legal_privacy_p5": "Използваме само строго необходими бисквитки за функционалността на платформата. За повече информация прегледайте настройките на браузъра си.",
    
    "legal_terms_title": "Общи условия",
    "legal_terms_last_updated": "Последно обновени: 28 Май 2026",
    "legal_terms_h2_1": "1. Приемане на условията",
    "legal_terms_p1": "Използвайки Bigla, вие се съгласявате с тези условия. Платформата е предназначена за управление на етажна собственост от упълномощени лица.",
    "legal_terms_h2_2": "2. Отговорности на потребителя",
    "legal_terms_p2": "Потребителите са отговорни за точността на въведените данни. Bigla не носи отговорност за спорове между живущите и домоуправителя.",
    "legal_terms_h2_3": "3. Прекратяване на достъп",
    "legal_terms_p3": "Запазваме си правото да прекратим достъпа на потребители, които злоупотребяват с платформата или нарушават приложимото законодателство.",
    "legal_terms_h2_4": "4. Интелектуална собственост",
    "legal_terms_p4": "Всички права върху софтуера, дизайна и марката Bigla са запазени. Копирането или разпространението им е строго забранено.",
    "legal_terms_h2_5": "5. Ограничаване на отговорността",
    "legal_terms_p5": "Платформата се предоставя \"във вида, в който е\". Ние не гарантираме 100% непрекъсваемост на услугата и не носим отговорност за пропуснати ползи."
  },
  en: {
    // Navbar
    "nav_features": "Features",
    "nav_how_it_works": "How it Works",
    "nav_pricing": "Pricing",
    "nav_get_demo": "Get Demo",

    // Hero
    "hero_badge": "🚀 The Future of Property Management",
    "hero_title_1": "Manage Apartment Finances ",
    "hero_title_gradient": "Effortlessly",
    "hero_subtitle": "Bigla is the ultimate SaaS platform for building cashiers. Track taxes, monitor expenses, and keep residents informed with unparalleled transparency.",
    "hero_btn_trial": "Get Demo",
    "hero_btn_features": "Explore Features",
    "hero_stat_1_val": "100%",
    "hero_stat_1_lbl": "Transparent",
    "hero_stat_2_val": "0",
    "hero_stat_2_lbl": "Spreadsheets",
    "hero_stat_3_val": "24/7",
    "hero_stat_3_lbl": "Cloud Access",

    // Features
    "feat_title": "Why Choose Bigla?",
    "feat_subtitle": "Built for cashiers and loved by residents. Stop chasing payments and start managing smarter.",
    "feat_1_title": "Real-time Tracking",
    "feat_1_desc": "Instant updates on who paid and who owes. Real-time sync means you always see the latest data.",
    "feat_2_title": "Expense Management",
    "feat_2_desc": "Easily log building expenses, repairs, and cleaning fees. Keep everyone accountable with clear financial balances.",
    "feat_3_title": "One-Click Reports",
    "feat_3_desc": "Generate detailed monthly reports with a single click and copy them directly to WhatsApp or Viber.",

    // How it Works
    "hiw_title": "How it Works",
    "hiw_subtitle": "Start managing your building in less than 5 minutes.",
    "hiw_step_1_title": "1. Register",
    "hiw_step_1_desc": "Register your building and set up the basic parameters and monthly fees.",
    "hiw_step_2_title": "2. Add Residents",
    "hiw_step_2_desc": "Enter the list of apartments and their individual fees.",
    "hiw_step_3_title": "3. Track & Report",
    "hiw_step_3_desc": "Mark payments with a single click and generate transparent reports for residents.",

    // Calculator
    "calc_title": "Save Time and Effort",
    "calc_slider_label": "Number of apartments: ",
    "calc_time_saved_1": "Saves you ~",
    "calc_time_saved_2": " of door-knocking and spreadsheet editing every month.",
    "calc_paper_saved_1": "Saves ",
    "calc_paper_saved_2": " paper receipts printed or handwritten per year.",
    "calc_price_label": "Monthly fee: ",

    // Pricing
    "pricing_title": "Simple & Transparent Pricing",
    "pricing_subtitle": "Choose the plan that fits your building's needs.",
    
    "price_demo_title": "Demo Version",
    "price_demo_price": "Free",
    "price_demo_feat_1": "Test all features",
    "price_demo_feat_2": "Up to 5 apartments",
    "price_demo_feat_3": "Limited reporting",
    "price_demo_btn": "Request Demo",

    "price_pro_title": "Premium",
    "price_pro_price": "From €5 / mo",
    "price_pro_feat_1": "€5 for first 10 apts, +€0.15 per extra",
    "price_pro_feat_2": "Full Viber/WhatsApp reports",
    "price_pro_feat_3": "Priority support",
    "price_pro_btn": "Get Premium",

    // FAQ
    "faq_title": "Frequently Asked Questions",
    "faq_subtitle": "Have questions? Here are the most common answers.",
    "faq_q1": "How long does setup take?",
    "faq_a1": "Setup takes less than 5 minutes. You just enter the number of apartments and monthly fees, and you're ready.",
    "faq_q2": "Do residents need to install an app?",
    "faq_a2": "No! Residents receive their clear monthly reports directly on Viber or WhatsApp as text messages.",
    "faq_q3": "Is the building's data secure?",
    "faq_a3": "Absolutely. All data is securely stored in the cloud with encryption, so only you have access to it.",

    // Contact / Demo Form
    "contact_title": "Request Your Demo",
    "contact_subtitle": "Leave your contact details and we'll get in touch to show you how Bigla can help your building.",
    "contact_lbl_name": "Full Name",
    "contact_lbl_email": "Email Address",
    "contact_lbl_phone": "Phone Number",
    "contact_btn_submit": "Submit Request",
    "contact_success": "Thank you! We will contact you shortly.",

    // Testimonials
    "testi_title": "What Cashiers Say",
    "testi_subtitle": "See how Bigla helps buildings like yours.",
    "testi_1_text": "\"Collecting fees used to take my whole weekend. Now it takes 15 minutes via Viber reports!\"",
    "testi_1_author": "Georgi Dimitrov",
    "testi_1_role": "Cashier, 45 apartments",
    "testi_2_text": "\"Residents finally see where the money goes. Transparency is at 100%.\"",
    "testi_2_author": "Maria Ivanova",
    "testi_2_role": "Cashier, 20 apartments",
    "testi_3_text": "\"I forgot about Excel spreadsheets. Everything is automated and easy to use.\"",
    "testi_3_author": "Petar Stoyanov",
    "testi_3_role": "Building Manager, 32 apartments",
    
    // Cookie Consent
    "cookie_msg": "This website uses cookies to ensure you get the best experience.",
    "cookie_accept": "Accept",
    "cookie_decline": "Decline",

    // Footer
    "footer_desc": "Smart property and tax management for modern communities.",
    "footer_github": "View on GitHub",
    "footer_privacy": "Privacy Policy",
    "footer_terms": "Terms of Service",
    "footer_rights": "© 2026 Bigla SaaS. All rights reserved.",

    // Legal Pages
    "legal_privacy_title": "Privacy Policy",
    "legal_privacy_last_updated": "Last updated: May 28, 2026",
    "legal_privacy_h2_1": "1. Introduction",
    "legal_privacy_p1": "At Bigla, we take your privacy seriously. This policy explains how we collect, use, and protect your personal information when using our platform.",
    "legal_privacy_h2_2": "2. Data Collection",
    "legal_privacy_p2": "We only collect information necessary to provide the service: names, emails, and apartment details. Payment data is processed securely by third parties.",
    "legal_privacy_h2_3": "3. Information Sharing",
    "legal_privacy_p3": "We never sell your data to third parties. All data is stored encrypted on secure servers in the EU.",
    "legal_privacy_h2_4": "4. Your Rights (GDPR)",
    "legal_privacy_p4": "You have the right to request access to your data, its correction, or deletion at any time. Please contact us for assistance.",
    "legal_privacy_h2_5": "5. Cookies",
    "legal_privacy_p5": "We only use strictly necessary cookies for the platform's functionality. For more information, check your browser settings.",
    
    "legal_terms_title": "Terms of Service",
    "legal_terms_last_updated": "Last updated: May 28, 2026",
    "legal_terms_h2_1": "1. Acceptance of Terms",
    "legal_terms_p1": "By using Bigla, you agree to these terms. The platform is intended for the management of residential buildings by authorized individuals.",
    "legal_terms_h2_2": "2. User Responsibilities",
    "legal_terms_p2": "Users are responsible for the accuracy of the entered data. Bigla is not liable for disputes between residents and building managers.",
    "legal_terms_h2_3": "3. Termination of Access",
    "legal_terms_p3": "We reserve the right to terminate access to users who abuse the platform or violate applicable laws.",
    "legal_terms_h2_4": "4. Intellectual Property",
    "legal_terms_p4": "All rights to the Bigla software, design, and brand are reserved. Copying or distributing them is strictly prohibited.",
    "legal_terms_h2_5": "5. Limitation of Liability",
    "legal_terms_p5": "The platform is provided \"as is\". We do not guarantee 100% service uptime and are not liable for lost profits."
  }
};
